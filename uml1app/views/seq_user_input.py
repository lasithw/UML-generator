                    ...
                    if (component.loop == "1" or component.If_loop == "1"):
                        os.write(fd, ("opt " + component.conditions + "\nloop\n" + str(
                            component.sender).lower() + "->" + (
                                          str(component.reciever).lower()).replace(" ","") + ":" + component.Message + "\nend\nend\n").encode('ascii'))
                    else:
                        os.write(fd,
            ...
            for values in list1:
                if (values['conditionBit'] != '1' and values['loop'] != '1'):
                    ...
                    if (values['else_loop'] != '1'):
                        ...
                        os.write(fd, b"else\n")
                        os.write(fd,
                                 ("[alt] : " + values['sender_else'] + "--->(" + values['elsemsg'] + "--->:" + values[
                                     'MessageType'] + ")---> " + values['reciver_else']).encode('ascii'))
                elif (values['loop'] == '1' and values['conditionBit'] == '1'):
                    if (values['else_loop'] != '1'):
                        os.write(fd, b"if\n")
                        os.write(fd,
                                 ("[opt] : [loop]:" + values['sender'] + "--->(" + values['Message'] + "--->:" + values[
                                     'MessageType'] + ")---> " + values['reciever']).encode('ascii'))
                    else:
                        os.write(fd, b"if\n")
                        os.write(fd,
                                 ("[alt] : [loop]:" + values['sender'] + "--->(" + values['Message'] + "--->:" + values[
                                     'MessageType'] + ")---> " + values['reciever']).encode('ascii'))
                        os.write(fd, b"\n")
                        os.write(fd, b"else\n")
                        os.write(fd, (
                                    "[alt] :[loop]: " + values['sender_else'] + "--->(" + values['elsemsg'] + "--->:" +
                                    values[
                                        'MessageType'] + ")---> " + values['reciver_else']).encode('ascii'))
            # Close opened file
            os.close(fd)
